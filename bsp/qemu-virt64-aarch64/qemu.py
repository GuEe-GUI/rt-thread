...
for i in range(len(opt)):
	...
	if is_opt("gl", inkey): q_gl = "-device virtio-gpu-gl-device -display {},gl=on ".format(opt[i + 1])
...
"""
def cmd():
	return cmd_base.format(RTT_QEMU_ROOT, smmu_cfg, q_gic, q_dumpdtb, q_el, q_smp, q_mem, q_bootargs, q_dtb, q_initrd,
		q_graphic, q_debug, q_block, q_net, q_scsi, cdrom_cfg, virtfs_cfg, plugged_mem_cfg,
		iommu_cfg, q_sound, q_flash, q_emmc, q_nvme, q_ahci, ufs_cfg, amp_cfg)
...
if len(q_dtbo) > 0:
	amp_mode = False
	# Read all dtbo
	with open("qemu.run.dtsi", 'w') as run_dtsi:
		for dtsi in q_dtbo:
			with open(dtsi) as dtsi:
				dts_content = dtsi.read()
				if not amp_mode and len(re.findall('amp_soc', dts_content)) > 0:
					amp_cfg += " -device loader,file=rtthread.bin,addr={}".format(re.findall(r'kernel-entry\s*=\s*<(\S+)>;', dts_content)[0])
					q_smp += 1
					amp_mode = True
				run_dtsi.write(dts_content)
	# Dump raw QEMU dts
	q_dumpdtb = ",dumpdtb=qemu.run.dtb"
	os.system(cmd())
	dumpdtb();
	os.remove("qemu.run.dtb")
	q_dumpdtb = ""
	# Merge all dtsi
	with open("qemu.run.dts", 'a') as dts:
		with open("qemu.run.dtsi", 'r') as dtsi:
			dts.write(dtsi.read())
	os.remove("qemu.run.dtsi")
	# Build runtime dtb
	dtc.dts_to_dtb(RTT_ROOT, ["qemu.run.dts"], ignore_warning = ["simple_bus_reg", "unit_address_vs_reg", "clocks_is_cell", "gpios_property"])
