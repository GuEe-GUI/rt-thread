...
static int k1x_sdhci_start_signal_voltage_switch(struct rt_mmc_host *mmc, struct rt_mmcsd_io_cfg *ios)
{
    ...
    case MMC_SIGNAL_VOLTAGE_330:
        ...
        if (!rt_is_err(mmc->rthost.supply.vqmmc))
        {
            if (sdio_regulator_set_vqmmc(&mmc->rthost, ios))
            {
                LOG_E("Switching to 3.3V signalling voltage failed");
                return -RT_EIO;
            }
        }
        /* Wait for 5ms */
        ...
        LOG_E("3.3V regulator output did not become stable");
        return -RT_EIO;
    ...
    case MMC_SIGNAL_VOLTAGE_180:
        ...
        if (!rt_is_err(mmc->rthost.supply.vqmmc))
        {
            if (sdio_regulator_set_vqmmc(&mmc->rthost, ios))
            {
                LOG_W("Switching to 1.8V signalling voltage failed");
                return -RT_EIO;
            }
        }
        ...
        LOG_W("1.8V regulator output did not become stable");
        return -RT_EIO;
    ...
    case MMC_SIGNAL_VOLTAGE_120:
        ...
        if (!rt_is_err(mmc->rthost.supply.vqmmc))
        {
            if (sdio_regulator_set_vqmmc(&mmc->rthost, ios))
            {
                LOG_W("Switching to 1.2V signalling voltage failed");
                return -RT_EIO;
            }
        }
...
static rt_err_t k1x_sdhci_probe(struct rt_platform_device *pdev)
{
    ...
    if (!rt_dm_dev_prop_read_u32(dev, "spacemit,tx_delaycode", &val))
    {
        pdata->tx_delaycode = (rt_uint8_t)val;
    }
    else
    {
