...
static rt_err_t ofw_append_gpio_led(struct rt_ofw_node *np)
{
    ...
    rt_dm_dev_set_name_auto(&gled->parent.parent, "led");
...
static int gpio_led_drv_register(void)
{
    rt_platform_driver_register(&gpio_led_driver);

    return 0;
}
INIT_DRIVER_LATER_EXPORT(gpio_led_drv_register);
