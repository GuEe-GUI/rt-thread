...
static rt_err_t syscon_led_probe(struct rt_platform_device *pdev)
{
    ...
    if (rt_ofw_prop_read_u32(np, "offset", &sled->offset))
    {
        goto _fail;
    }

    if (rt_ofw_prop_read_u32(np, "mask", &sled->mask))
    {
        goto _fail;
    }
    ...
    rt_dm_dev_set_name_auto(&sled->parent.parent, "led");

    if ((err = rt_hw_led_register(&sled->parent)))
    {
        goto _fail;
    }
...
static rt_err_t syscon_led_remove(struct rt_platform_device *pdev)
{
    ...
    rt_hw_led_unregister(&sled->parent);
...
static int syscon_led_drv_register(void)
{
    rt_platform_driver_register(&syscon_led_driver);

    return 0;
}
INIT_DRIVER_LATER_EXPORT(syscon_led_drv_register);
