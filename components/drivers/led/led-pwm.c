...
static rt_err_t pwm_led_set_brightness(struct rt_led_device *led, rt_uint32_t brightness)
{
    ...
    pled->pwm_conf.pulse = rt_pwm_conf_pulse(&pled->pwm_conf, duty);
...
static rt_err_t ofw_append_pwm_led(struct rt_ofw_node *np)
{
    ...
    struct pwm_led *pled = rt_malloc(sizeof(*pled));
    ...
    pled->pwm_dev = rt_ofw_data(pwm_args.data);
    ...
    rt_dm_dev_set_name_auto(&pled->parent.parent, "led");

    if ((err = rt_hw_led_register(&pled->parent)))
    {
        goto _fail;
    }
    ...
    if (!rt_ofw_prop_read_string(np, "default-state", &state))
    {
        ...
        else if (!rt_strcmp(state, "keep"))
        {
            if (pled->pwm_conf.period)
            {
                ...
                brightness *= rt_pwm_conf_duty_cycle(&pled->pwm_conf);
...
static rt_err_t pwm_led_remove(struct rt_platform_device *pdev)
{
    ...
    rt_ofw_foreach_available_child_node(np, led_np)
    {
        ...
        rt_hw_led_unregister(&pled->parent);
...
static int pwm_led_drv_register(void)
{
    rt_platform_driver_register(&pwm_led_driver);

    return 0;
}
INIT_DRIVER_LATER_EXPORT(pwm_led_drv_register);
