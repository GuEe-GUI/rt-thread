...
static rt_err_t arm_arch_timer_ofw_init(struct rt_platform_device *pdev, int irq_idx)
{
    rt_err_t err = RT_EOK;
    const char *irq_name[] =
    {
    ...
    if (np)
    {
        arm_arch_timer_irq = rt_ofw_get_irq_by_name(np, irq_name[irq_idx]);

        if (arm_arch_timer_irq < 0)
        {
            arm_arch_timer_irq = rt_ofw_get_irq(np, irq_idx);
        }
    }
...
static rt_err_t arm_arch_timer_probe(struct rt_platform_device *pdev)
{
    ...
    err = arm_arch_timer_ofw_init(pdev, irq_idx);
