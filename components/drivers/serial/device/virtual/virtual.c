...
static int virtual_serial_getc(struct rt_serial_device *serial)
{
    ...
    ch = rt_device_read(vs->kdbdev, 0, &kdb_ev, 1);
    ...
    if (kdb_ev.code == KEY_UP)
    {
        vs->ctrl = 0;
        goto _ctrl;
    }
    else if (kdb_ev.code == KEY_LEFT)
    {
        vs->ctrl = 1;
        goto _ctrl;
    }
    else if (kdb_ev.code == KEY_RIGHT)
    {
        vs->ctrl = 2;
        goto _ctrl;
    }
    else if (kdb_ev.code == KEY_DOWN)
    {
        vs->ctrl = 3;
        goto _ctrl;
    }

    ch = key_map[kdb_ev.code];
...
static int virtual_serial_setup(void)
{
    ...
    if ((err = render_load_font(virtual_font, sizeof(virtual_font),
    ...
    if (_vs.kdbdev)
    {
        struct rt_device_notify kdb_notify;

        kdb_notify.notify = &virtual_serial_kdb_notify;
        kdb_notify.dev = &_vs.parent.parent;

        rt_device_open(_vs.kdbdev, 0);
        rt_device_control(_vs.kdbdev, RT_DEVICE_CTRL_NOTIFY_SET, &kdb_notify);
    }
...
INIT_APP_EXPORT(virtual_serial_setup);
