...
static void *last_buffer_multi(void)
{
    ...
    return (void *)rd->fix.smem_start + rd->buffer_idx * rd->line_size;
}
...
static void fb_move_buffer_multi(void *dst, void *src, rt_size_t size)
{
    ...
    if (rt_unlikely(rd->buffer_idx == 0))
    {
...
static void fb_pan_display(void)
{
    ...
    rd->var.yoffset = rd->buffer_idx * rd->line_size;
        ...
        struct render_point *out_start_point, struct render_point *out_end_point)
{
    ...
    if (rd->buffer_size)
    {
        rd->var.yres_virtual = rd->buffer_size * rd->var.yres;
...
void render_move_cursor(struct render_point *position)
{
    ...
    if (position)
    {
        ...
        if (rt_unlikely(position->row > rd->end_point.row || position->col > rd->end_point.col))
        {
            return;
        }
