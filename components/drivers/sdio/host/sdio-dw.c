...
rt_inline void sdio_dw_set_cto(struct sdio_dw *sd)
{
    ...
    if (!bitmap_test_bit(&sd->pending_events, EVENT_CMD_COMPLETE))
    {
...
static void sdio_dw_stop_dma(struct sdio_dw *sd)
{
    ...
    bitmap_set_bit(&sd->pending_events, EVENT_XFER_COMPLETE);
}
...
static rt_uint32_t sdio_dw_prep_stop_abort(struct sdio_dw *sd, struct rt_mmcsd_cmd *cmd)
{
    ...
    if (!bitmap_test_bit(&sd->slot->flags, DW_MMC_CARD_NO_USE_HOLD))
    {
...
static void sdio_dw_setup_bus(struct sdio_dw_slot *slot, rt_bool_t force_clkinit)
{
    ...
    else if (clock != sd->current_speed || force_clkinit)
    {
        ...
        if (clock != slot->clk_old &&
            !bitmap_test_bit(&slot->flags, DW_MMC_CARD_NEEDS_POLL) &&
            !force_clkinit)
        {
        ...
        if (!bitmap_test_bit(&slot->flags, DW_MMC_CARD_NO_LOW_PWR))
        {
...
static void sdio_dw_start_request(struct sdio_dw *sd, struct rt_mmcsd_cmd *cmd)
{
    ...
    if (!bitmap_test_bit(&slot->flags, DW_MMC_CARD_NO_USE_HOLD))
    {
        cmd_flags |= SDIO_DW_CMD_USE_HOLD_REG;
    }

    if (bitmap_test_bit(&slot->flags, DW_MMC_CARD_NEED_INIT))
    {
        cmd_flags |= SDIO_DW_CMD_INIT;
        bitmap_clear_bit(&slot->flags, DW_MMC_CARD_NEED_INIT);
    }
...
static void sdio_dw_mmc_set_iocfg(struct rt_mmcsd_host *host, struct rt_mmcsd_io_cfg *ios)
{
    ...
    case MMCSD_POWER_UP:
        ...
        bitmap_set_bit(&slot->flags, DW_MMC_CARD_NEED_INIT);
...
static void sdio_dw_mmc_enable_sdio_irq(struct rt_mmcsd_host *host, rt_int32_t enable)
{
    ...
    if (enable)
    {
        bitmap_set_bit(&slot->flags, DW_MMC_CARD_NO_LOW_PWR);
        clk_en_a = clk_en_a_old & ~clken_low_pwr;
    }
    else
    {
        bitmap_clear_bit(&slot->flags, DW_MMC_CARD_NO_LOW_PWR);
        clk_en_a = clk_en_a_old | clken_low_pwr;
    }
...
static void sdio_dw_set_drto(struct sdio_dw *sd)
{
    ...
    if (!bitmap_test_bit(&sd->pending_events, EVENT_DATA_COMPLETE))
    {
...
static rt_bool_t sdio_dw_clear_pending_cmd_complete(struct sdio_dw *sd)
{
    ...
    bitmap_clear_bit(&sd->pending_events, EVENT_CMD_COMPLETE);
...
static rt_bool_t sdio_dw_clear_pending_data_complete(struct sdio_dw *sd)
{
    ...
    bitmap_clear_bit(&sd->pending_events, EVENT_DATA_COMPLETE);
...
_next_status:
    ...
    case STATE_SENDING_DATA:
        if (bitmap_test_bit(&sd->pending_events, EVENT_DATA_ERROR))
        {
            bitmap_clear_bit(&sd->pending_events, EVENT_DATA_ERROR);
        ...
        if (!bitmap_test_bit(&sd->pending_events, EVENT_XFER_COMPLETE))
        {
            /*
        ...
        bitmap_clear_bit(&sd->pending_events, EVENT_XFER_COMPLETE);
...
static void sdio_dw_cmd11_timer(void *param)
{
    ...
    bitmap_set_bit(&sd->pending_events, EVENT_CMD_COMPLETE);
    rt_workqueue_urgent_work(sd->state_wq, &sd->state_work);
}
...
static void sdio_dw_dto_timer(void *param)
{
    ...
    case STATE_DATA_BUSY:
        ...
        bitmap_set_bit(&sd->pending_events, EVENT_DATA_ERROR);
        bitmap_set_bit(&sd->pending_events, EVENT_DATA_COMPLETE);
        rt_workqueue_urgent_work(sd->state_wq, &sd->state_work);
        break;
