...
rt_inline rt_bool_t sdhci_has_requests(struct rt_sdhci_host *host)
{
    return host->cmd || host->data_cmd;
}

rt_inline rt_bool_t sdhci_auto_cmd23(struct rt_sdhci_host *host,
                                     struct rt_mmcsd_req *mrq)
{
    return mrq->sbc && (host->flags & RT_SDHCI_AUTO_CMD23);
}

rt_inline rt_bool_t sdhci_auto_cmd12(struct rt_sdhci_host *host,
                                     struct rt_mmcsd_req *mrq)
{
    return !mrq->sbc && (host->flags & RT_SDHCI_AUTO_CMD12) && !mrq->cap_cmd_during_tfr;
}

rt_inline rt_bool_t sdhci_manual_cmd23(struct rt_sdhci_host *host,
                                       struct rt_mmcsd_req *mrq)
{
    return mrq->sbc && !(host->flags & RT_SDHCI_AUTO_CMD23);
}

rt_inline rt_bool_t sdhci_data_line_cmd(struct rt_mmcsd_cmd *cmd)
{
...
rt_inline rt_bool_t sdhci_can_64bit_dma(struct rt_sdhci_host *host)
{
...
void rt_sdhci_cleanup_host(struct rt_sdhci_host *host)
{
    ...
    {
        rt_regulator_disable(mmc->rthost.supply.vqmmc);
    }
}

static void sdhci_set_default_irqs(struct rt_sdhci_host *host)
{
    host->ier = RT_SDHCI_INT_BUS_POWER | RT_SDHCI_INT_DATA_END_BIT |
                RT_SDHCI_INT_DATA_CRC | RT_SDHCI_INT_DATA_TIMEOUT |
                RT_SDHCI_INT_INDEX | RT_SDHCI_INT_END_BIT | RT_SDHCI_INT_CRC |
                RT_SDHCI_INT_TIMEOUT | RT_SDHCI_INT_DATA_END | RT_SDHCI_INT_RESPONSE;
...
rt_inline void sdhci_auto_cmd_select(struct rt_sdhci_host *host,
...
{
    return (!(host->flags & RT_SDHCI_DEVICE_DEAD) &&
                ((mrq->cmd && mrq->cmd->err) || (mrq->sbc && mrq->sbc->err) ||
                    (mrq->data && mrq->data->stop && mrq->data->stop->err) ||
                        (host->quirks & RT_SDHCI_QUIRK_RESET_AFTER_REQUEST)));
}
...
static void sdhci_reset_for_reason(struct rt_sdhci_host *host,
...
{
    rt_sdhci_writel(host, rt_lower_32_bits(addr), RT_SDHCI_ADMA_ADDRESS);

    if (host->flags & RT_SDHCI_USE_64_BIT_DMA)
    {
        rt_sdhci_writel(host, rt_upper_32_bits(addr), RT_SDHCI_ADMA_ADDRESS_HI);
    }
}
...
rt_inline void sdhci_set_block_info(struct rt_sdhci_host *host,
                                    struct rt_mmcsd_data *data)
{
    int boundary;
    rt_size_t total_size = data->blks * data->blksize;
    ...
    rt_sdhci_writew(host, RT_SDHCI_MAKE_BLKSZ(boundary, data->blksize),
                    RT_SDHCI_BLOCK_SIZE);

    if (host->version >= RT_SDHCI_SPEC_410 &&
        host->v4_mode && (host->quirks2 & RT_SDHCI_QUIRK2_USE_32BIT_BLK_CNT))
    {
...
static void sdhci_set_mrq_done(struct rt_sdhci_host *host, struct rt_mmcsd_req *mrq)
{
    ...
    RT_ASSERT(!(i >= RT_SDHCI_MAX_MRQS));
}

rt_inline rt_bool_t sdhci_defer_done(struct rt_sdhci_host *host,
                                     struct rt_mmcsd_req *mrq)
{
    struct rt_mmcsd_data *data = mrq->data;

    return host->pending_reset || host->always_defer_done ||
            ((host->flags & RT_SDHCI_REQ_USE_DMA) && data &&
...
{
    return !(host->quirks2 & RT_SDHCI_QUIRK2_PRESET_VALUE_BROKEN) &&
                                ...
                                unsigned short vdd)
{
    ...
    sdio_regulator_set_ocr(&mmc->rthost, mmc->rthost.supply.vmmc, vdd);
...
void rt_sdhci_set_power_and_bus_voltage(struct rt_sdhci_host *host, unsigned char mode,
                                        unsigned short vdd)
{
...
void rt_sdhci_set_power(struct rt_sdhci_host *host, unsigned char mode,
                        unsigned short vdd)
{
    if (!host->mmc->rthost.supply.vmmc)
    {
        rt_sdhci_set_power_with_noreg(host, mode, vdd);
    }
    else
                                         ...
                                         struct rt_mmcsd_io_cfg *ios)
{
    ...
    case MMC_SIGNAL_VOLTAGE_330:
        ...
        if (!mmc->rthost.supply.vqmmc)
        {
            ret = sdio_regulator_set_vqmmc(&mmc->rthost, ios);
            if (ret < 0)
            {
                return -EIO;
            }
...
static int sdhci_check_ro(struct rt_sdhci_host *host)
{
    ...
    {
        is_readonly = !(rt_sdhci_readl(host, RT_SDHCI_PRESENT_STATE) & RT_SDHCI_WRITE_PROTECT);
    }
                                         ...
                                         struct rt_mmcsd_data *data)
{
    ...
    {
        target_timeout = RT_DIV_ROUND_UP(data->timeout_ns, 1000);

        if (host->clock && data->timeout_clks)
        {
            rt_uint32_t val = 1000000ULL * data->timeout_clks;

            if (rt_do_div(val, host->clock))
            {
                target_timeout++;
            }
...
static rt_uint8_t sdhci_calc_timeout(struct rt_sdhci_host *host,
                                     struct rt_mmcsd_cmd *cmd,
                                     rt_bool_t *too_big)
{
    rt_uint8_t count;
                                  ...
                                  struct rt_mmcsd_cmd *cmd)
{
    ...
    unsigned int blksz, freq;
    rt_uint64_t target_timeout, transfer_time;
    struct rt_mmc_host *mmc = host->mmc;
    struct rt_mmcsd_data *data = cmd->data;
    struct rt_mmcsd_io_cfg *ios = &mmc->ios;

    bus_width       = 1 << ios->bus_width;
    target_timeout  = sdhci_target_timeout(host, cmd, data);
...
static void __sdhci_finish_mrq(struct rt_sdhci_host *host, struct rt_mmcsd_req *mrq)
{
    ...
    {
        host->data_cmd = RT_NULL;
    }

    if (host->deferred_cmd && host->deferred_cmd->mrq == mrq)
    {
        host->deferred_cmd = RT_NULL;
    }
                                    ...
                                    struct rt_mmcsd_cmd *cmd)
{
    ...
    if (!data)
    {
        ...
        else
        {
            ...
            rt_sdhci_writew(host, mode & ~(RT_SDHCI_TRNS_AUTO_CMD12 | RT_SDHCI_TRNS_AUTO_CMD23),
...
static rt_bool_t sdhci_send_command(struct rt_sdhci_host *host, struct rt_mmcsd_cmd *cmd)
{
    ...
    if ((host->quirks2 & RT_SDHCI_QUIRK2_STOP_WITH_TC) &&
    ...
    else if (!cmd->data && (int)cmd->busy_timeout > 9000)
    {
        timeout += (RT_DIV_ROUND_UP(cmd->busy_timeout, 1000) + 1) * RT_TICK_PER_SECOND;
    }
    else
    {
...
static void sdhci_data_irq(struct rt_sdhci_host *host, rt_uint32_t intmask)
{
    ...
    else if ((intmask & RT_SDHCI_INT_DATA_CRC) &&
             RT_SDHCI_GET_CMD(rt_sdhci_readw(host, RT_SDHCI_COMMAND)) != MMC_BUS_TEST_R)
    {
        host->data->err = -EILSEQ;
    }
    ...
    else
    {
        ...
        if (intmask & RT_SDHCI_INT_DMA_END)
        {
            ...
            dmanow   = (dmanow & ~((rt_uint32_t)RT_SDHCI_DEFAULT_BOUNDARY_SIZE - 1));
            dmanow  += RT_SDHCI_DEFAULT_BOUNDARY_SIZE;
            host->data->bytes_xfered = dmanow - dmastart;

            LOG_D("DMA base %p, transferred 0x%06x bytes, next %p",
        ...
        if (intmask & RT_SDHCI_INT_DATA_END)
        {
            ...
            if (data->buf)
            {
                if (mmc_get_dma_dir(data) == DMA_FROM_DEVICE)
                {
                    rt_hw_cpu_dcache_ops(RT_HW_CACHE_INVALIDATE, data->buf,
                                         data->blks * data->blksize);
                }
                else
                {
                    rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, data->buf,
                                         data->blks * data->blksize);
                }
            }
...
static void sdhci_cmd_irq(struct rt_sdhci_host *host,
                          rt_uint32_t intmask, rt_uint32_t *intmask_p)
{
    if (intmask & RT_SDHCI_INT_AUTO_CMD_ERR && host->data_cmd)
    {
        int data_err_bit;
        rt_uint16_t auto_cmd_status;
        struct rt_mmcsd_req *mrq = host->data_cmd->mrq;

        auto_cmd_status = rt_sdhci_readw(host, RT_SDHCI_AUTO_CMD_STATUS);
        data_err_bit    = (auto_cmd_status & RT_SDHCI_AUTO_CMD_TIMEOUT) ?
    ...
    if (intmask & RT_SDHCI_INT_AUTO_CMD_ERR)
    {
        ...
        auto_cmd_status = rt_sdhci_readw(host, RT_SDHCI_AUTO_CMD_STATUS);
        err             = (auto_cmd_status & RT_SDHCI_AUTO_CMD_TIMEOUT) ? -ETIMEDOUT : -EILSEQ;
...
#define IRQ_WAIT 1
#define IRQ_DONE 2
    int max_loops = 16, result = IRQ_NONE;
    ...
    do {
        LOG_D("IRQ status 0x%08x", intmask);
        ...
        if (intmask & (RT_SDHCI_INT_CARD_INSERT | RT_SDHCI_INT_CARD_REMOVE))
        {
            ...
            rt_sdhci_writel(host, intmask & (RT_SDHCI_INT_CARD_INSERT | RT_SDHCI_INT_CARD_REMOVE),
                            RT_SDHCI_INT_STATUS);
        ...
        intmask &= ~(RT_SDHCI_INT_CARD_INSERT | RT_SDHCI_INT_CARD_REMOVE |
                     RT_SDHCI_INT_CMD_MASK | RT_SDHCI_INT_DATA_MASK |
                     RT_SDHCI_INT_ERROR | RT_SDHCI_INT_BUS_POWER |
                     RT_SDHCI_INT_RETUNE | RT_SDHCI_INT_CARD_INT);
    ...
    for (int i = 0; i < RT_SDHCI_MAX_MRQS; ++i)
    {
...
static rt_bool_t rt_sdhci_start_request_done(struct rt_sdhci_host *host)
{
    ...
    if (host->flags & RT_SDHCI_REQ_USE_DMA)
    {
        ...
        if (data && data->host_cookie == RT_SDHCI_COOKIE_MAPPED)
        {
...
{
    for (int i = 0; i < host->tuning_loop_count; ++i)
    {
        rt_uint16_t ctrl;
...
/*                     error                               */
/********************************************************* */
static const struct rt_mmc_host_ops rt_sdhci_ops =
{
                             ...
                             unsigned int *actual_clock)
{
    ...
    int div = 0, real_div = 0, clk_mul = 1;
    ...
    if (host->version >= RT_SDHCI_SPEC_300)
    {
        if (host->preset_enabled)
        {
            ...
            div     = RT_FIELD_GET(RT_SDHCI_PRESET_SDCLK_FREQ_MASK, pre_val);
        ...
        if (!host->clk_mul || switch_base_clk)
        {
            ...
            {
                for (div  = 2; div < RT_SDHCI_MAX_DIV_SPEC_300; div += 2)
                {
...
{
    rt_uint16_t v;
    rt_uint64_t dt_caps_mask = 0, dt_caps = 0;
    ...
    else
    {
        ...
        host->caps &= ~rt_lower_32_bits(dt_caps_mask);
        host->caps |= rt_lower_32_bits(dt_caps);
    }
    ...
    else
    {
        ...
        host->caps1 &= ~rt_upper_32_bits(dt_caps_mask);
        host->caps1 |= rt_upper_32_bits(dt_caps);
    }
}
                                          ...
                                          rt_size_t priv_size)
{
    ...
    host->tuning_delay      = -1;
    host->tuning_loop_count = RT_SDHCI_MAX_TUNING_LOOP;
...
int rt_sdhci_setup_host(struct rt_sdhci_host *host)
{
    ...
    rt_size_t max_current_caps, max_clk;
    unsigned int ocr_avail, override_timeout_clk;
    struct rt_mmc_host *mmc;
    ...
    if (!mmc->rthost.supply.vqmmc)
    {
        ret = sdio_regulator_get_supply(mmc->parent,&mmc->rthost);
        if (ret)
        {
            return ret;
        }
    ...
    if (!(host->flags & RT_SDHCI_USE_SDMA))
    {
        host->dma_mask = RT_DMA_ADDR_MASK(64);
    }
    if (host->version >= RT_SDHCI_SPEC_300)
    {
        host->max_clk = RT_FIELD_GET(RT_SDHCI_CLOCK_V3_BASE_MASK, host->caps);
    }
    else
    {
        host->max_clk = RT_FIELD_GET(RT_SDHCI_CLOCK_BASE_MASK, host->caps);
    }
    ...
    host->clk_mul = RT_FIELD_GET(RT_SDHCI_CLOCK_MUL_MASK, host->caps1);
    ...
    {
        host->timeout_clk = RT_FIELD_GET(RT_SDHCI_TIMEOUT_CLK_MASK, host->caps);
        ...
        if (host->timeout_clk == 0)
        {
            ...
            host->timeout_clk = RT_DIV_ROUND_UP(host->ops->get_timeout_clock(host), 1000);
        }
    ...
    if ((host->version >= RT_SDHCI_SPEC_300) &&
        (!(host->flags & RT_SDHCI_USE_SDMA) || host->v4_mode) &&
            !(host->quirks2 & RT_SDHCI_QUIRK2_ACMD23_BROKEN))
    {
    ...
    if ((host->quirks & RT_SDHCI_QUIRK_BROKEN_CARD_DETECTION) &&
        mmc_card_is_removable(mmc) && rt_mmc_gpio_get_cd(mmc) < 0)
    {
    ...
    if (mmc->rthost.supply.vqmmc)
    {
        if (enable_vqmmc)
        {
            rt_regulator_enable(mmc->rthost.supply.vqmmc);
            host->sdhci_core_to_disable_vqmmc = !ret;
        }

        if (!rt_regulator_is_supported_voltage(mmc->rthost.supply.vqmmc, 1700000, 1950000))
        {
            host->caps1 &= ~(RT_SDHCI_SUPPORT_SDR104 | RT_SDHCI_SUPPORT_SDR50 | RT_SDHCI_SUPPORT_DDR50);
        }

        if (!rt_regulator_is_supported_voltage(mmc->rthost.supply.vqmmc, 2700000, 3600000))
        {
            host->flags &= ~RT_SDHCI_SIGNALING_330;
        }
        ...
        if (ret)
        {
            ...
            mmc->rthost.supply.vqmmc = (void *)-EINVAL;
        }
    }
    ...
    if ((host->quirks2 & RT_SDHCI_QUIRK2_CAPS_BIT63_FOR_HS400) &&
        (host->caps1 & RT_SDHCI_SUPPORT_HS400))
    {
        mmc->caps2 |= MMC_CAP2_HS400;
    }
    if ((mmc->caps2 & MMC_CAP2_HSX00_1_2V) && (!mmc->rthost.supply.vqmmc ||
            !rt_regulator_is_supported_voltage(mmc->rthost.supply.vqmmc, 1100000, 1300000)))
    {
    ...
    host->tuning_count = RT_FIELD_GET(RT_SDHCI_RETUNING_TIMER_COUNT_MASK, host->caps1);
    ...
    host->tuning_mode = RT_FIELD_GET(RT_SDHCI_RETUNING_MODE_MASK, host->caps1);

    ocr_avail = 0;

    max_current_caps = rt_sdhci_readl(host, RT_SDHCI_MAX_CURRENT);

    if (!max_current_caps && mmc->rthost.supply.vmmc)
    {
        int curr = rt_regulator_get_voltage(mmc->rthost.supply.vmmc);
        if (curr > 0)
        {
            curr = curr / 1000; /* convert to mA */
            curr = curr / RT_SDHCI_MAX_CURRENT_MULTIPLIER;

            curr = rt_min_t(rt_uint32_t, curr, RT_SDHCI_MAX_CURRENT_LIMIT);
            max_current_caps = RT_FIELD_PREP(RT_SDHCI_MAX_CURRENT_330_MASK, curr) |
                               RT_FIELD_PREP(RT_SDHCI_MAX_CURRENT_300_MASK, curr) |
                               RT_FIELD_PREP(RT_SDHCI_MAX_CURRENT_180_MASK, curr);
        }
    }
    ...
    if (host->caps & RT_SDHCI_CAN_VDD_330)
    {
        ...
        mmc->max_current_330  = RT_FIELD_GET(RT_SDHCI_MAX_CURRENT_330_MASK, max_current_caps);
        mmc->max_current_330 *= RT_SDHCI_MAX_CURRENT_MULTIPLIER;
    }
    if (host->caps & RT_SDHCI_CAN_VDD_300)
    {
        ocr_avail |= MMC_VDD_29_30 | MMC_VDD_30_31;

        mmc->max_current_300  = RT_FIELD_GET(RT_SDHCI_MAX_CURRENT_300_MASK, max_current_caps);
        mmc->max_current_300 *= RT_SDHCI_MAX_CURRENT_MULTIPLIER;
    }
    if (host->caps & RT_SDHCI_CAN_VDD_180)
    {
        ocr_avail |= MMC_VDD_165_195;

        mmc->max_current_180  = RT_FIELD_GET(RT_SDHCI_MAX_CURRENT_180_MASK, max_current_caps);
        mmc->max_current_180 *= RT_SDHCI_MAX_CURRENT_MULTIPLIER;
    }
    ...
    if ((mmc->caps & (MMC_CAP_UHS_SDR12 | MMC_CAP_UHS_SDR25 | MMC_CAP_UHS_SDR50 |
                      MMC_CAP_UHS_SDR104 | MMC_CAP_UHS_DDR50 | MMC_CAP_1_8V_DDR)) ||
...
int rt_sdhci_init_host(struct rt_sdhci_host *host)
{
    ...
    host->complete_wq = rt_workqueue_create(dev_name, RT_SYSTEM_WORKQUEUE_STACKSIZE, 20);
    if (!host->complete_wq)
    {
        return -ENOMEM;
    }
...
{
    rt_uint8_t ctrl;
    rt_bool_t reinit_uhs, turning_on_clk = RT_FALSE;
    struct rt_sdhci_host *host = rt_mmc_priv(mmc);

    reinit_uhs       = host->reinit_uhs;
    ...
    if (host->flags & RT_SDHCI_DEVICE_DEAD)
    {
        if (mmc->rthost.supply.vmmc && ios->power_mode == MMC_POWER_OFF)
        {
            sdio_regulator_set_ocr(&mmc->rthost, mmc->rthost.supply.vmmc, 0);
        }
    ...
    if (!reinit_uhs && turning_on_clk &&
        host->timing == ios->timing && host->version >= RT_SDHCI_SPEC_300 &&
    ...
    if (host->version >= RT_SDHCI_SPEC_300)
    {
        ...
        if (sdhci_preset_needed(host, ios->timing))
        {
            ...
            ios->drv_type  = RT_FIELD_GET(RT_SDHCI_PRESET_DRV_MASK, preset);
            host->drv_type = ios->drv_type;
        }
