...
static rt_err_t ads7846_probe(struct rt_spi_device *spi_dev)
{
    ...
    /* For legacy ti,swap-xy */
    if (rt_dm_dev_prop_read_bool(dev, "touchscreen-swapped-x-y") &&
        ts->pdata.swap_xy)
    {
        struct rt_input_absinfo absinfo;

        rt_memcpy(&absinfo, &ts->parent.absinfo[ABS_X], sizeof(absinfo));
        rt_memcpy(&ts->parent.absinfo[ABS_X], &ts->parent.absinfo[ABS_Y], sizeof(absinfo));
        rt_memcpy(&ts->parent.absinfo[ABS_Y], &absinfo, sizeof(absinfo));
    }
    ...
    if ((ts->irq = rt_dm_dev_get_irq(dev, 0)))
    {
        err = ts->irq;
