...
static void e3x0_button_release_isr(int irqno, void *param)
{
    ...
    /* Key up */
    rt_thread_resume(btn->poweroff_task);
}

static void e3x0_button_power_off_task(void *param)
{
    struct e3x0_button *btn = param;

    rt_thread_suspend(btn->poweroff_task);
    rt_schedule();
...
static rt_err_t e3x0_button_probe(struct rt_platform_device *pdev)
{
    ...
    btn->poweroff_task = rt_thread_create("button-e3x0", &e3x0_button_power_off_task,
            btn, SYSTEM_THREAD_STACK_SIZE, RT_THREAD_PRIORITY_MAX / 2, 10);

    if (!btn->poweroff_task)
    {
        goto _fail;
    }

    rt_thread_startup(btn->poweroff_task);
...
static rt_err_t e3x0_button_remove(struct rt_platform_device *pdev)
{
    ...
    rt_thread_delete(btn->poweroff_task);
