...
#define DBG_TAG "video.backlight.pwm"
...
static rt_err_t pwm_backlight_update_status(struct rt_backlight_device *bl)
{
    ...
    else
    {
        ...
        pwm_conf.pulse = rt_pwm_conf_pulse(&pwm_conf, 0);
        rt_pwm_set(pbl->pwm_dev, pwm_conf.channel, pwm_conf.period, pwm_conf.pulse);
...
static rt_err_t pwm_backlight_probe(struct rt_platform_device *pdev)
{
    ...
    if (pbl->enable_pin < 0 && pbl->enable_pin != -RT_EEMPTY)
    {
        err = pbl->enable_pin;
    ...
    pbl->power_supply = rt_regulator_get_optional(dev, "power");
    ...
    pbl->pwm_dev = rt_ofw_data(pwm_args.data);
    ...
    rt_dm_dev_set_name_auto(&pbl->parent.parent, "backlight");

    if ((err = rt_hw_backlight_register(&pbl->parent)))
    {
        goto _fail;
    }
...
static rt_err_t pwm_backlight_remove(struct rt_platform_device *pdev)
{
    ...
    rt_hw_backlight_unregister(&pbl->parent);
