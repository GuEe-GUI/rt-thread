...
#define DBG_TAG "video.backlight.gpio"
...
static rt_err_t gpio_backlight_probe(struct rt_platform_device *pdev)
{
    ...
    rt_dm_dev_set_name_auto(&gbl->parent.parent, "backlight");

    if ((err = rt_hw_backlight_register(&gbl->parent)))
    {
        goto _fail;
    }
...
static rt_err_t gpio_backlight_remove(struct rt_platform_device *pdev)
{
    ...
    rt_hw_backlight_unregister(&gbl->parent);
