        ...
        rt_uint32_t params_nr, const struct optee_msg_param *msg_params)
{
    for (int n = 0; n < params_nr; ++n, ++raw_params, ++msg_params)
    {
        ...
        case OPTEE_MSG_ATTR_TYPE_RMEM_INOUT:
            raw_params->attr = OPTEE_MSG_ATTR_TYPE_RMEM_INPUT;
...
static void supplicant_cmd_shm_free(struct optee_msg_arg *msg_arg)
{
    msg_arg->ret_origin = RT_TEE_ORIGIN_COMMS;
    ...
    rt_tee_shm_free_helper((void *)msg_arg->params[0].value.b);
    msg_arg->ret = RT_TEE_SUCCESS;
}
        ...
        void **page_list)
{
    ...
    case OPTEE_MSG_RPC_CMD_SHM_FREE:
        supplicant_cmd_shm_free(msg_arg);
        break;
        ...
        struct optee_msg_arg *msg_arg)
{
    ...
    while (RT_TRUE)
    {
        ...
        case OPTEE_SMC_RPC_FUNC_ALLOC:
            ...
            if (!rt_tee_shm_alloc_helper(shm))
            {
