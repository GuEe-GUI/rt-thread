...
rt_err_t rt_nvmem_device_register(struct rt_nvmem_device *ndev)
{
    ...
    if (!ndev->ignore_wp)
    {
        ndev->wp_pin = rt_pin_get_named_pin(&ndev->parent, "wp", 0,
                RT_NULL, &ndev->wp_pin_active);

        if (ndev->wp_pin < 0 && ndev->wp_pin != -RT_EEMPTY)
        {
            return -RT_EINVAL;
        }
    }
        ...
        rt_size_t len)
{
    ...
    if (rt_unlikely(!cell || !buffer || !len))
    {
        return -RT_EINVAL;
    }
    ...
    if (rt_unlikely(len > nvmem->size || len > cell->bytes))
    {
        return -RT_EINVAL;
    }

    if (rt_unlikely(!nvmem->reg_read))
    {
        return -RT_ENOSYS;
    }

    res = nvmem->reg_read(nvmem, cell->offset, buffer, len);
    ...
    if (cell->bit_offset || cell->nbits)
    {
        ...
        extra = cell->bytes - RT_DIV_ROUND_UP(cell->nbits, BITS_PER_BYTE);
        while (--extra >= 0)
        {
            *p-- = 0;
        }
        ...
        {
            *p &= RT_GENMASK((cell->nbits % BITS_PER_BYTE) - 1, 0);
        }
    }
        ...
        rt_size_t len)
{
    ...
    if (rt_unlikely(!nvmem->reg_write))
    {
        return -RT_ENOSYS;
    }
    ...
    if (cell->bit_offset || cell->nbits)
    {
        ...
        if ((nbits + bit_offset) % BITS_PER_BYTE)
        {
            ...
            *p |= RT_GENMASK(7, (nbits + bit_offset) % BITS_PER_BYTE) & v;
        }
        ...
        int index, const char *const_id)
{
    ...
    const fdt32_t *addr;
    struct rt_nvmem_device *nvmem;
    struct rt_nvmem_cell *cell = RT_NULL;
    ...
    rt_list_for_each_entry(cell, &nvmem->cell_nodes, list)
    {
        if (cell->index == index)
        {
            if (const_id && cell->id && rt_strcmp(const_id, cell->id))
            {
                continue;
            }

            if (ref_read(&cell->ref))
            {
                ref_get(&cell->ref);
            }
            else
...
static void nvmem_release(struct ref *r)
{
...
static void cell_release(struct ref *r)
{
    rt_ubase_t level;
    ...
    ref_put(&nvmem->ref, &nvmem_release);
