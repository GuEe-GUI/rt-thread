        ...
        rt_size_t bytes)
{
    ...
    {
        bitmap_set_bit(rk_otp->wp_mask, bit_idx + i);
    }
        ...
        void *val, rt_size_t bytes)
{
    ...
    if (!rk_otp->was_writed && rk_otp->soc_data && rk_otp->soc_data->write)
    {
...
static rt_err_t rockchip_otp_probe(struct rt_platform_device *pdev)
{
    ...
    if ((err = rt_nvmem_device_register(&rk_otp->parent)))
    {
        goto _fail;
    }
