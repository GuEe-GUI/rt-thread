...
#define raw_to_rk_clk(raw) rt_container_of(raw, struct rk_clk, clk_parent)
...
static rt_err_t clk_rk3568_probe(struct rt_platform_device *pdev)
{
    ...
    if (rk_clk)
    {
        ...
        rt_memset(&rk_clk->clk_parent, 0, sizeof(rk_clk->clk_parent));
        ...
        rk_clk->clk_parent.ops = &rk3568_clk_ops;

        if ((err = rt_clk_register(&rk_clk->clk_parent, RT_NULL)))
        {
            goto _fail;
        }

        if ((err = rk_register_softrst(&rk_clk->rstc_parent, np,
                softrst_regs, ROCKCHIP_SOFTRST_HIWORD_MASK)))
        {
            goto _fail;
        }

        rt_ofw_data(np) = &rk_clk->clk_parent;
    }
    else
    {
        err = -RT_ENOMEM;
