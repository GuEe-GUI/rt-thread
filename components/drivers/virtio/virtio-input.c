...
static rt_err_t virtio_input_vq_init(struct virtio_input *vinput)
{
    ...
    vinput->event_nr = rt_virtqueue_get_virtq_size(vinput->vqs[QUEUE_EVENTS]);

    vinput->events = rt_malloc(sizeof(*vinput->events) * vinput->event_nr);
    ...
    for (int i = 0; i < vinput->event_nr; ++i)
    {
