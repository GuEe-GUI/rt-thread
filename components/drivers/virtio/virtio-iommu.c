...
#define DBG_TAG "virtio.dev.iommu"
#define DBG_LVL DBG_INFO
#include <rtdbg.h>

#include "virtio_config/virtio-iommu.h"
...
static rt_err_t virtio_iommu_attach(struct rt_iommu_device *iommu, struct rt_iommu_domain *domain)
{

}

static rt_err_t virtio_iommu_detach(struct rt_iommu_device *iommu, struct rt_iommu_domain *domain)
{

}

static rt_err_t virtio_iommu_map(struct rt_iommu_device *iommu, struct rt_iommu_domain *domain,
        void *iopa, void *iova, rt_size_t size, mm_flag_t flags)
{

}

static rt_err_t virtio_iommu_unmap(struct rt_iommu_device *iommu, struct rt_iommu_domain *domain,
        void *iova, rt_size_t size)
{

}

static rt_err_t virtio_iommu_iotlb_flush(struct rt_iommu_device *iommu, struct rt_iommu_domain *domain)
{

}

static rt_err_t virtio_iommu_iotlb_sync(struct rt_iommu_device *iommu, struct rt_iommu_domain *domain,
        void *iova, rt_size_t size)
{

}

static int virtio_iommu_ofw_parse(struct rt_iommu_device *iommu, struct rt_iommu_domain *domain,
        struct rt_ofw_cell_args *iommu_args)
{

}

static struct rt_iommu_ops virtio_iommu_ops =
{
    .attach = virtio_iommu_attach,
    .detach = virtio_iommu_detach,
    .map = virtio_iommu_map,
    .unmap = virtio_iommu_unmap,
    .iotlb_flush = virtio_iommu_iotlb_flush,
    .iotlb_sync = virtio_iommu_iotlb_sync,
    .ofw_parse = virtio_iommu_ofw_parse,
};

static rt_err_t virtio_iommu_probe(struct rt_virtio_device *vdev)
{
    return RT_EOK;
}

static rt_err_t virtio_iommu_remove(struct rt_virtio_device *vdev)
{
    return RT_EOK;
}

static const struct rt_virtio_device_id virtio_iommu_ids[] =
{
    { VIRTIO_DEVICE_ID_IOMMU, VIRTIO_DEVICE_ANY_ID },
    { /* sentinel */ }
};

static struct rt_virtio_driver virtio_iommu_driver =
{
    .ids = virtio_iommu_ids,
    .features =
        RT_BIT(VIRTIO_IOMMU_F_MAP_UNMAP)
      | RT_BIT(VIRTIO_IOMMU_F_INPUT_RANGE)
      | RT_BIT(VIRTIO_IOMMU_F_DOMAIN_RANGE)
      | RT_BIT(VIRTIO_IOMMU_F_PROBE)
      | RT_BIT(VIRTIO_IOMMU_F_MMIO)
      | RT_BIT(VIRTIO_IOMMU_F_BYPASS_CONFIG)
      | RT_BIT(VIRTIO_F_ANY_LAYOUT),

    .probe = virtio_iommu_probe,
    .remove = virtio_iommu_remove,
};
RT_VIRTIO_DRIVER_EXPORT(virtio_iommu_driver);
#if 0
