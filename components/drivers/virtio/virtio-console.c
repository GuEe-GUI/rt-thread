...
#ifdef RT_USING_DFS_DIRECTFS
    struct rt_object port_obj;
    struct rt_object port_name;
#endif
    char name[64];
...
#ifdef RT_USING_DFS_DIRECTFS
static rt_ssize_t console_port_name_read_raw(rt_object_t obj,
        struct directfs_bin_attribute *attr,
        char *buffer, rt_off_t pos, rt_size_t count)
{
    struct console_port *port = rt_container_of(obj, struct console_port, port_name);

    if (pos < sizeof(port->name))
    {
        count = rt_min_t(rt_size_t, count, sizeof(port->name) - pos);
        rt_strncpy(buffer, &port->name[pos], count);
    }
    else
    {
        count = -RT_EINVAL;
...
static struct directfs_bin_attribute console_port_name_directfs_attr_raw =
{
    .attr =
    {
        .name = "raw",
    },
    .read = console_port_name_read_raw,
};
...
#ifdef RT_USING_DFS_DIRECTFS
    dfs_directfs_create_link(&vconsole->vdev->parent.parent, &port->port_obj, name);
    dfs_directfs_create_link(&port->port_obj, &port->port_name, "name");
    dfs_directfs_create_bin_file(&port->port_name, &console_port_name_directfs_attr_raw);
#endif
...
#ifdef RT_USING_DFS_DIRECTFS
    dfs_directfs_destroy_link(&port->port_name);
    dfs_directfs_destroy_link(&port->port_obj);
#endif
