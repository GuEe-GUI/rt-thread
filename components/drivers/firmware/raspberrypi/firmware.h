...
#ifndef __RASPBERRYPI_FIRMWARE_H__
#define __RASPBERRYPI_FIRMWARE_H__
...
struct rpi_firmware_property_tag_header
{
    ...
    rt_uint32_t req_resp_size;  /* always 4 byte aligned */
};
...
enum rpi_firmware_property_tag
{
    ...
    RPI_FIRMWARE_GET_RTC_REG =                            0x00030087,
    RPI_FIRMWARE_SET_RTC_REG =                            0x00038087,
...
{
    rt_le32_t id;   /* ID of the clock being queried */
    rt_le32_t rate; /* Rate in Hertz. Set by the firmware */
} rt_packed;

#define RPI_FIRMWARE_CLK_RATE_REQUEST(ID)   \
{                                           \
    .id = ID,                               \
}

rt_err_t rpi_firmware_property(struct rpi_firmware *rpi_fw, rt_uint32_t tag, void *tag_data, rt_size_t buf_size);
rt_err_t rpi_firmware_property_list(struct rpi_firmware *rpi_fw, void *data, rt_size_t tag_size);
...
#endif /* __RASPBERRYPI_FIRMWARE_H__ */
