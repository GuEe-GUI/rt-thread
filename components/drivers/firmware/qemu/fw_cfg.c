...
static rt_err_t ramfb_control(rt_device_t dev, int cmd, void *args)
{
    ...
    case FBIOGET_VSCREENINFO:
        ...
        rt_memcpy(args, &fbdev->var, sizeof(fbdev->var));
        break;
    ...
    case FBIOPUT_VSCREENINFO:
    {
        ...
        struct fb_var_screeninfo *usr_var = args;
        ...
        /* Check is init */
        if (usr_var->xres == fbdev->var.xres &&
            usr_var->yres == fbdev->var.yres &&
            usr_var->bits_per_pixel == fbdev->var.bits_per_pixel &&
            usr_var->grayscale == fbdev->var.grayscale &&
            !rt_memcmp(&usr_var->red, &fbdev->var.red, sizeof(usr_var->red)) &&
            !rt_memcmp(&usr_var->green, &fbdev->var.green, sizeof(usr_var->green)) &&
            !rt_memcmp(&usr_var->blue, &fbdev->var.blue, sizeof(usr_var->blue)) &&
            !rt_memcmp(&usr_var->transp, &fbdev->var.transp, sizeof(usr_var->transp)))
        {
            if (usr_var->yres_virtual > fbdev->var.yres &&
                usr_var->yres_virtual <= fbdev->var.yres * fbdev->buffer_size)
            {
                fbdev->var.yres_virtual = usr_var->yres_virtual;
            }
        ...
        {
            if (usr_var->red.offset != 11 || usr_var->red.length != 5 ||
                usr_var->green.offset != 5 || usr_var->green.length != 6 ||
                usr_var->blue.offset != 0 || usr_var->blue.length != 5)
            {
                err = -RT_EINVAL;
                break;
            }

            format = pixman('R', 'G', '1', '6');
        }
        else if (bpp == 15)
        {
            if (usr_var->red.offset != 10 || usr_var->red.length != 5)
            {
                err = -RT_EINVAL;
                break;
            }

            if (usr_var->transp.length)
            {
                if (usr_var->transp.length != 1 || usr_var->transp.offset != 15)
                {
                    err = -RT_EINVAL;
                    break;
                }
            ...
            format = pixman(usr_var->transp.length ? 'A' : 'X', 'R', '1', '5');
        }
        else
        {
            err = -RT_EINVAL;
        ...
        if (res >= 0)
        {
            if (fbdev->fix.smem_start)
            {
                rt_free_align((void *)fbdev->fix.smem_start);
            }

            fbdev->fix.smem_start = (rt_ubase_t)fb;
            fbdev->fix.smem_len = fb_len;
            fbdev->fix.line_length = (bpp / 8) * usr_var->xres;
            fbdev->fix.mmio_start = (rt_ubase_t)rt_kmem_v2p(fb);
            fbdev->fix.mmio_len = usr_var->yres * fbdev->fix.line_length;

            rt_memcpy(&fbdev->var, usr_var, sizeof(fbdev->var));
    ...
    case FBIOGET_FSCREENINFO:
        ...
        rt_memcpy(args, &fbdev->fix, sizeof(fbdev->fix));
        break;
    ...
    case FBIOPAN_DISPLAY:
    {
        ...
        if (usr_var->yoffset < fbdev->var.yres_virtual &&
            usr_var->yoffset != fbdev->var.yoffset)
        {
            rt_base_t res;
            struct fw_cfg_ram_fb ram_fb;
            void *fb = (void *)fbdev->fix.smem_start + usr_var->yoffset;

            ram_fb.addr   = rt_cpu_to_be64((rt_ubase_t)rt_kmem_v2p(fb));
            ram_fb.fourcc = rt_cpu_to_be32(fbdev->format);
            ram_fb.flags  = rt_cpu_to_be32(0);
            ram_fb.width  = rt_cpu_to_be32(fbdev->var.xres);
            ram_fb.height = rt_cpu_to_be32(fbdev->var.yres);
            ram_fb.stride = rt_cpu_to_be32((fbdev->var.bits_per_pixel / 8) * fbdev->var.xres);

            res = fw_cfg_write_blob(rt_be16_to_cpu(fbdev->file->select), &ram_fb, 0, sizeof(struct fw_cfg_ram_fb));

            if (res >= 0)
            {
                fbdev->var.yoffset = usr_var->yoffset;
                err = RT_EOK;
            }
            else
            {
