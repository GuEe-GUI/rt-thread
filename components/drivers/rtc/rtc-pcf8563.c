        ...
        struct rt_rtc_wkalarm *alarm)
{
    ...
    buf[2] = rt_bin2bcd(0); /* alarm->tm_mday */
...
static rt_err_t pcf8563_clkout_init(struct rt_clk *clk, void *fw_data)
{
    rt_err_t err;
    rt_uint8_t buf;
    struct pcf8563_rtc *pcf8563 = raw_to_pcf8563_clkout(clk->clk_np);
...
static rt_err_t pcf8563_clkout_prepare(struct rt_clk *clk)
{
    struct pcf8563_rtc *pcf8563 = raw_to_pcf8563_clkout(clk->clk_np);
...
static void pcf8563_clkout_unprepare(struct rt_clk *clk)
{
    struct pcf8563_rtc *pcf8563 = raw_to_pcf8563_clkout(clk->clk_np);
...
static rt_bool_t pcf8563_clkout_is_prepared(struct rt_clk *clk)
{
    rt_uint8_t buf;
    struct pcf8563_rtc *pcf8563 = raw_to_pcf8563_clkout(clk->clk_np);
...
static rt_err_t pcf8563_clkout_set_rate(struct rt_clk *clk, rt_ubase_t rate,
        rt_ubase_t parent_rate)
{
    ...
    for (int i = 0; i < RT_ARRAY_SIZE(clkout_rates); ++i)
    {
        if (clkout_rates[i] == rate)
        {
            ...
            err = pcf8563_write_block_data(pcf8563, PCF8563_REG_CLKO, 1, &buf);
...
static rt_base_t pcf8563_clkout_round_rate(struct rt_clk *clk, rt_ubase_t drate,
        rt_ubase_t *prate)
{
    for (int i = 0; i < RT_ARRAY_SIZE(clkout_rates); ++i)
    {
...
static void pcf8563_clkout_register_clk(struct pcf8563_rtc *pcf8563)
{
    rt_uint8_t buf;
    ...
    pcf8563->clkout_hw.ops = &pcf8563_clkout_ops;

    if (rt_clk_register(&pcf8563->clkout_hw, RT_NULL))
    {
        return;
    }

    rt_dm_dev_bind_fwdata(&pcf8563->client->parent, RT_NULL, &pcf8563->clkout_hw);
}
...
static rt_err_t pcf8563_rtc_probe(struct rt_i2c_client *client)
{
    ...
    if (pcf8563->irq >= 0)
    {
        pcf8563->irq_thread = rt_thread_create("rtc-pcf8563", &pcf8563_rtc_thread_isr,
                pcf8563, SYSTEM_THREAD_STACK_SIZE, RT_THREAD_PRIORITY_MAX / 2, 10);
