...
static rt_err_t hym8563_clkout_init(struct rt_clk *clk, void *fw_data)
{
    int res;
    struct hym8563_rtc *hym8563 = raw_to_hym8563_clkout(clk->clk_np);
...
static rt_err_t hym8563_clkout_prepare(struct rt_clk *clk)
{
    struct hym8563_rtc *hym8563 = raw_to_hym8563_clkout(clk->clk_np);
...
static void hym8563_clkout_unprepare(struct rt_clk *clk)
{
    struct hym8563_rtc *hym8563 = raw_to_hym8563_clkout(clk->clk_np);
...
static rt_bool_t hym8563_clkout_is_prepared(struct rt_clk *clk)
{
    int res;
    struct hym8563_rtc *hym8563 = raw_to_hym8563_clkout(clk->clk_np);
...
static rt_err_t hym8563_clkout_set_rate(struct rt_clk *clk, rt_ubase_t rate,
...
static rt_base_t hym8563_clkout_round_rate(struct rt_clk *clk, rt_ubase_t drate,
        rt_ubase_t *prate)
{
    for (int i = 0; i < RT_ARRAY_SIZE(clkout_rates); ++i)
    {
...
static void hym8563_clkout_register_clk(struct hym8563_rtc *hym8563)
{
    ...
    hym8563->clkout_hw.ops = &hym8563_clkout_ops;

    if (rt_clk_register(&hym8563->clkout_hw, RT_NULL))
    {
        return;
    }

    rt_dm_dev_bind_fwdata(&hym8563->client->parent, RT_NULL, &hym8563->clkout_hw);
}
