        ...
        struct rt_rtc_wkalarm *alarm)
{
    ...
    alarmvals[2] = rt_bin2bcd(0);   /* alarm->tm_mday */
    ...
    if (alarm->enable)
    {
        rx8010->ctrlreg |= RX8010_CTRL_UIE | RX8010_CTRL_AIE | RX8010_CTRL_UIE;
...
static rt_err_t rx8010_rtc_probe(struct rt_i2c_client *client)
{
    ...
    if (rx8010->irq >= 0)
    {
        rx8010->irq_thread = rt_thread_create("rtc-rx8010", &rx8010_rtc_thread_isr,
                rx8010, SYSTEM_THREAD_STACK_SIZE, RT_THREAD_PRIORITY_MAX / 2, 10);
