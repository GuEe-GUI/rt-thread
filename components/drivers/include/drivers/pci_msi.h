 ...
 *                           |   |         |             |      |
 *                           |   |         |             |      +---- MSI Enable (RW)
 *                           |   |         |             +----------- Multiple Message Capable (RO, 2 ^ n [n <= 5])
 *                           |   |         +------------------------- Multiple Message Enable (RW, 2 ^ n [n <= 5])
...
struct rt_pci_msi_conf
{
    rt_uint32_t mask;
    rt_uint8_t mask_pos;
    rt_uint8_t pending_pos;

    struct
    {
        rt_uint8_t is_masking:1;
        rt_uint8_t is_64bit:1;
        rt_uint8_t multi_msg_max:3;
        rt_uint8_t multi_msg_use:3;
    } cap;
};
...
#define rt_msi_for_each_desc(pdev, desc) \
    rt_list_for_each_entry(desc, &pdev->msi_desc_nodes, list)

#define rt_msix_table_size(flags) ((flags & PCIM_MSIXCTRL_TABLE_SIZE) + 1)
