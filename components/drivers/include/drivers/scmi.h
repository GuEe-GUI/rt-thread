 ...
 *
 * @rx_callback:    SCMI P2A delayed responses callback
 ...
 * @num_domains:        Number of power domains
 * @stats_addr_low:     The lower 32 bits of the physical address where the statistics shared memory region is located
 * @stats_addr_high:    The upper 32 bits of the physical address where the statistics shared memory region is located
 * @stats_size:         The length in bytes of the statistics shared memory region
 */
 ...
 * @flags:              Async flag
 * @domain:             Identifier for the power domain
 * @state:              Platform-specific parameter identifying the power state of the domain
 */
...
/**
 * struct scmi_power_state_set_out - Response payload for SCMI_POWER_STATE_GET command
 * @status:             SCMI command status
 */
 ...
 * @max_async_req:      Maximum number of pending asynchronous clock rate changes supported by the platform
 */
 ...
 * @id:                 Identifier for the clock device
 * @rate_index:         Index to the first rate value to be described in the return rate array
 */
 ...
 * @status:             SCMI command status
 * @num_rates_flags:    Bits[31:16] Number of remaining rates, Bit[12] flags
 * @value_low:          Lower 32 bits of the physical rate in Hertz
 * @value_high:         Upper 32 bits of the physical rate in Hertz
 */
 ...
 * @max_requests:   Number of max requests
 * @reg_addr_low:   This value indicates the lower 32 bits of the physical address where the sensor shared memory region is located
 * @reg_addr_high:  This value indicates the upper 32 bits of the physical address where the shared memory region is located
 * @reg_size:       This value indicates the length in bytes of the shared memory region
 */
 ...
 * @desc_index:         Index of the first sensor descriptor to be read in the sensor descriptor array.
 */
 ...
 * @status:             SCMI command status
 * @num_sensor_flags:   Number of axis and flags
 * @id:                 ID of the sensor
 * @attributes_low:     Lower 32 bits of the attributes
 * @attributes_high:    Higher 32 bits of the attributes
 * @name:               Name of the sensor
 * @power:              The average power consumed by the sensor in microwatts (uW) when it is active
 * @resolution:         The resolution of the sensor
 * @min_range_low:      Lower 32 bits of the min range
 * @min_range_high:     Higher 32 bits of the min range
 * @max_range_low:      Lower 32 bits of the max range
 * @max_range_high:     Higher 32 bits of the max range
 */
 ...
 * @id:                 ID of the sensor
 * @axis_desc_index:    Index of the first sensor axis descriptor to be read in the sensor axis descriptor array.
 */
 ...
 * @status:                 SCMI command status
 * @num_axis_flags:         Number of axis and flags
 * @desc:                   An array of sensor axis descriptors of format
 * @axis_id:                ID of the axis
 * @axis_attributes_low:    Lower 32 bits of the attributes
 * @axis_attributes_high:   Higher 32 bits of the attributes
 * @name:                   Axis name
 */
 ...
 * @id:                 ID of the sensor
 * @index:              Index to the first update interval value to be described in the return interval array
 */
 ...
 * @id:                     ID of the sensor
 * @trip_point_ev_ctrl:     Event control
 * @trip_point_val_low:     Lower 32 bits of the sensor value
 * @trip_point_val_high:    Higher 32 bits of the sensor value
 */
 ...
 * @id:                 ID of sensor
 * @flags:              Flags for the sensor request
 */
 ...
 * @value_low:          Lower 32 bits of the sensor value
 * @value_high:         Upper 32 bits of the sensor value
 * @timestamp_low:      Lower 32 bits of the sensor timestamp
 * @timestamp_high:     Upper 32 bits of the sensor timestamp
 */
 ...
 * @id:                 ID of the sensor
 * @axis_id:            Index of the first sensor axis to be read in the sensor axis name descriptor array.
 */
 ...
 * @status:             SCMI command status
 * @flags:              Flags for the sensor request
 * @name:               An array of axis name descriptors
 * @axis_id:            ID of the axis
...
/**
 * struct scmi_reset_in - Message payload for SCMI_RESET command
 * @domain_id:      SCMI reset domain ID
...
/**
 * struct scmi_reset_out - Response payload for SCMI_RESET command
 * @status:         SCMI command status
 */
...
#define SCMI_VOLTAGE_NAME_LEN       16
 ...
 * @status:         SCMI command status
 * @num_domains:    Number of voltage domains
 */
...
struct scmi_voltage_attr_out
{
    rt_le32_t status;
    rt_le32_t attributes;
    char name[SCMI_VOLTAGE_NAME_LEN];
};

/**
 * struct scmi_voltage_config_set_in - Message payload for SCMI_VOLTAGE_CONFIG_SET cmd
 * @domain_id:      SCMI voltage domain ID
...
/**
 * struct scmi_voltage_config_set_out - Response for SCMI_VOLTAGE_CONFIG_SET command
 * @status:         SCMI command status
 */
...
/**
 * struct scmi_voltage_level_set_in - Message payload for SCMI_VOLTAGE_LEVEL_SET cmd
 * @domain_id:      SCMI voltage domain ID
...
/**
 * struct scmi_voltage_level_set_out - Response for SCMI_VOLTAGE_LEVEL_SET command
 * @status:     SCMI command status
 */
...
/**
 * struct scmi_voltage_level_get_in - Message payload for SCMI_VOLTAGE_LEVEL_GET cmd
 * @domain_id:  SCMI voltage domain ID
 */
...
/**
 * struct scmi_voltage_level_get_out - Response for SCMI_VOLTAGE_LEVEL_GET command
 * @status:         SCMI command status
 ...
 * @status:             SCMI command status
 * @attributes_low:     Number of pin groups, pins
 * @attributes_high:    Number of functions
 */
 ...
 * @identifier: Identifier for the pin, group, or function
 * @flags:      Selector of enum scmi_pinctrl_selector_type
 */
 ...
 * @status:     SCMI command status
 * @attributes: The attributes of the pin, group, or function identified
 * @name:       Null-terminated ASCII string of up to 16 bytes in length describing the pin, group, or function name
 */
 ...
 * @identifier: Identifier for the function or group
 * @flags:      Selector of enum scmi_pinctrl_selector_type (PIN is Reserved)
 * @index:      Index of the first pin to be described in the return array
 */
 ...
 * @status: SCMI command status
 * @flags:  Number of remaining groups or pins/identifiers
 * @array:  Array of group or pin identifiers listed in numerically ascending order
 */
 ...
 * @identifier: Identifier for the function or group
 * @attributes: Settings information
 */
 ...
 * @status:             SCMI command status
 * @function_selected:  The function currently selected to be enabled by the pin or group specified in the input identifier field
 * @num_configs:        Number of remaining configurations/configurations that are returned by this call
 * @configs:            Array of configurations, sorted in numerically increasing config type order
 */
 ...
 * @identifier:     Identifier for the function or group
 * @function_id:    Identifier for the function selected to be enabled for the selected pin or group
 * @attributes:     Settings information
 * @configs:        Array of configs to set, sorted in numerically increasing config type order
 */
 ...
 * @identifier: Identifier for the function or group
 * @flags:      Selector of enum scmi_pinctrl_selector_type (FUNCTION is Reserved)
 */
 ...
 * @name:   Null-terminated ASCII string of up to 64 bytes in length  describing the extended pin, group, or function name
 */
