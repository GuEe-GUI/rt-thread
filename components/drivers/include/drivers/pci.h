...
struct rt_pci_device
{
    ...
    rt_uint8_t msi_cap;
    rt_uint8_t msix_cap;
    DECLARE_BITMAP(flags, PCI_FLAGS_NR);
...
{
    return bitmap_test_bit((bitmap_t *)pdev->flags, flag);
}

rt_inline void rt_pci_device_set_flag(struct rt_pci_device *pdev, int flag)
{
    bitmap_set_bit(pdev->flags, flag);
}
...
{
    bitmap_clear_bit(pdev->flags, flag);
}
...
{
    return bitmap_test_bit(pdev->flags, bit);
}
...
#ifdef RT_PCI_MSI
rt_err_t rt_pci_alloc_vector(struct rt_pci_device *pdev, int min, int max, rt_uint32_t flags, bitmap_t *affinity);
...
rt_err_t rt_pci_msi_enable(struct rt_pci_device *pdev);

rt_ssize_t rt_pci_msix_vector_count(struct rt_pci_device *pdev);
rt_err_t rt_pci_msix_disable(struct rt_pci_device *pdev);
rt_err_t rt_pci_msix_enable_range(struct rt_pci_device *pdev, struct rt_pci_msix_entry *entries, int min, int max);
#else
rt_err_t rt_pci_alloc_vector(struct rt_pci_device *pdev, int min, int max, rt_uint32_t flags, bitmap_t *affinity)
{
    return -RT_ENOSYS;
...
rt_inline rt_err_t rt_pci_msix_enable(struct rt_pci_device *pdev, struct rt_pci_msix_entry *entries, int count)
{
