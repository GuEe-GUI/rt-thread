...
static int pl330_cmd_dmamov(rt_uint8_t *microcode, rt_uint8_t rd, rt_uint32_t imm)
{
    ...
    *microcode++ = rd;
    imm = rt_cpu_to_le32(imm);
    rt_memcpy(microcode, &imm, 4);
...
for (int i = 0; i < pc->microcode_len; i++)
{

    rt_kprintf("[%02d] = %02x\n", i, ((rt_uint8_t *)pc->microcode)[i]);
}
    chan_id = pc - pl330->chans;
    rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, pc->microcode, pc->microcode_len);

    HWREG32(pl330->regs + PL330_REG_DBGINST0) = ((chan_id + 1) << 8) | (PL330_CMD_DMAGO << 16);
...
static rt_err_t pl330_dma_stop(struct rt_dma_chan *chan)
{
    ...
    HWREG32(pl330->regs + PL330_REG_DBGINST0) = ((chan_id + 1) << 8) | PL330_CMD_DMAKILL;
        ...
        rt_ubase_t dma_addr_src, rt_ubase_t dma_addr_dst, rt_size_t len)
{
    ...
    loop = len / ((1 << conf->src_addr_width) * conf->src_maxburst);

    ljmp1 = mc - pc->microcode;
    mc += pl330_cmd_dmalp(mc, 0, loop);
...
{
    rt_uint8_t *mc;
    rt_size_t loop_count;
    rt_uint32_t sar, dar, ccr;
    struct pl330_chan *pc = rt_container_of(chan, struct pl330_chan, parent);
    struct rt_dma_slave_config *conf = &chan->conf;

    if (period_len == 0 || buf_len % period_len != 0)
    {
        LOG_E("Cyclic error: buf_len = %lu period_len = %lu", buf_len, period_len);
        return -RT_EINVAL;
    }

    loop_count = buf_len / period_len;

    if (loop_count > 255)
    {
        LOG_E("DMALP loop count cannot exceed 255 (now: %lu)", loop_count);
        return -RT_EINVAL;
    }
    ...
    ccr = (conf->src_maxburst << 14) | (conf->dst_maxburst << 11);
    ...
    *mc++ = PL330_CMD_DMAST;
    ...
    /* DMALPEND */
    *mc++ = PL330_CMD_DMALPEND;

    *mc++ = PL330_CMD_DMAEND;

    pc->size = buf_len;
    pc->microcode_len = mc - (rt_uint8_t *)pc->microcode;
        ...
        enum rt_dma_transfer_direction dir)
{
    ...
    *mc++ = 0x02; /* CCR */
    ...
    for (int i = buf_len / ((1 << conf->src_addr_width) * conf->src_maxburst); i > 0; i--)
    {
        *mc++ = PL330_CMD_DMALD;
        *mc++ = PL330_CMD_DMAST;
    }
...
rt_inline rt_ubase_t rt_hw_timer_count(void)
{
    rt_ubase_t count;
...
static int test_dma(void)
{
    ...
    static struct rt_dma_slave_config config;
    static struct rt_dma_slave_transfer transfer;
    struct rt_dma_chan *chn = rt_dma_chan_request(&dev, RT_NULL);

    rt_ubase_t dma_addr;
    static char *src_addr, *dst_addr;

    src_addr = rt_dma_alloc_coherent(&dev, 32, &dma_addr);
    config.src_addr = dma_addr;
    dst_addr = rt_dma_alloc_coherent(&dev, 32, &dma_addr);
    ...
    rt_kprintf("config.src_addr = %p config.dst_addr = %p\n", config.src_addr, config.dst_addr);

    rt_dma_chan_config(chn, &config);

    rt_memset(&transfer, 0, sizeof(transfer));
    transfer.src_addr = config.src_addr;
    transfer.dst_addr = config.dst_addr;
    transfer.buffer_len = 32;

    rt_dma_prep_memcpy(chn, &transfer);

    rt_memset(src_addr, 0xff, 32);

    for (int i = 0; i < 32; ++i)
    {
        rt_kprintf("%02x ", src_addr[i]);
    }
    rt_kprintf("end0 src_addr start print\n");
    for (int i = 0; i < 32; ++i)
    {
        rt_kprintf("%02x ", dst_addr[i]);
    }
    rt_kprintf("end0 dst_addr start print\n");

    rt_ubase_t last = rt_hw_timer_count();
    rt_dma_chan_start(chn);
    rt_kprintf("DMA count = %u\n", rt_hw_timer_count() - last);
    ...
    rt_kprintf("end0 src_addr end print\n");
    for (int i = 0; i < 32; ++i)
    {
        rt_kprintf("%02x ", dst_addr[i]);
    }
    rt_kprintf("end0 dst_addr end print\n\n");
    ...
    rt_kprintf("end1 src_addr start print\n");
    for (int i = 0; i < 32; ++i)
    {
        rt_kprintf("%02x ", dst_addr[i]);
    }
    rt_kprintf("end1 dst_addr start print\n");

    last = rt_hw_timer_count();
    rt_memcpy(dst_addr, src_addr, 32);
    rt_kprintf("MEMCPY count = %u\n", rt_hw_timer_count() - last);

    for (int i = 0; i < 32; ++i)
    {
        rt_kprintf("%02x ", src_addr[i]);
    }
    rt_kprintf("end1 src_addr end print\n");
    for (int i = 0; i < 32; ++i)
    {
        rt_kprintf("%02x ", dst_addr[i]);
    }
    rt_kprintf("end1 dst_addr end print\n");

    rt_dma_free_coherent(&dev, 32, src_addr, config.src_addr);
    rt_dma_free_coherent(&dev, 32, dst_addr, config.dst_addr);

    return 0;
}
MSH_CMD_EXPORT(test_dma, test_dma);
