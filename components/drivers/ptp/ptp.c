...
rt_err_t rt_ptp_clock_unregister(struct rt_ptp_clock *ptp)
{
    ...
    rt_memset(&perout.period, 0, sizeof(perout.period));
    perout.flags = PTP_PEROUT_ONE_SHOT;

    for (int i = 0; ptp->perout_nr; ++i)
    {
        perout.chan = i;
...
rt_err_t rt_ptp_adjtime(struct rt_ptp_clock *ptp, struct rt_ptp_clock_time *ts)
{
    ...
    if ((rt_int64_t)ts->nsec >= NSEC_PER_SEC)
    {
        return -RT_EINVAL;
    }
        ...
        rt_uint32_t func, rt_uint32_t chan)
{
    ...
    case PTP_PIN_FUNC_EXTTS:
        request.extts.chan = chan;
        err = rt_ptp_request_extts(ptp, &request.extts);
        break;

    case PTP_PIN_FUNC_PEROUT:
        request.perout.chan = chan;
        err = rt_ptp_request_perout(ptp, &request.perout);
        break;
