...
{
    struct rt_dvfs_opp *opp, *opp_last = RT_NULL, *opp_next;
    ...
    rt_list_for_each_entry_safe(opp, opp_next, &opp_table->opp_nodes, list)
    {
        ...
        if (dir > 0)
        {
            if (opp->freq > freq)
            {
                if (!opp_last || freq > opp_last->freq)
                {
                    return opp;
                }
            }
        ...
        else if (dir < 0)
        {
            if (opp->freq < freq)
            {
                if (&opp_next->list == &opp_table->opp_nodes || freq < opp_next->freq)
                {
                    return opp;
                }
            }
...
struct rt_dvfs_opp *rt_dvfs_scaling_add_opp(struct rt_dvfs_scaling *dvfs, rt_ubase_t freq, rt_ubase_t uvolt)
{
    ...
    struct rt_dvfs_opp *opp, *opp_next, *opp_last = RT_NULL;
    ...
    rt_list_for_each_entry(opp_next, &opp_table->opp_nodes, list)
    {
        if (opp->freq < opp_next->freq)
        {
            if (opp_last)
            {
                rt_list_insert_after(&opp_last->list, &opp->list);
            }
    ...
    else if (&opp_next->list == &opp_table->opp_nodes)
    {
