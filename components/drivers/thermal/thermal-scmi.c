        ...
        int *out_temp)
{
    ...
    struct rt_scmi_msg msg = RT_SCMI_MSG_IN(SCMI_COM_MSG_ATTRIBUTES, &reading_in, &reading_out);
    ...
    for (rt_uint64_t scale = (st->scale + 3) - 1; scale >= 0; --scale)
    {
        div *= 10;
    }

    *out_temp = rt_div_u64(value, div);
...
static rt_err_t scmi_thermal_probe(struct rt_scmi_device *sdev)
{
    ...
    struct rt_scmi_msg msg = RT_SCMI_MSG_IN(SCMI_COM_MSG_ATTRIBUTES, &attr, &attr);
    ...
    for (int i = 0, ts_nr = 0; i < attr.num_sensors; ++i)
    {
        ...
        msg = RT_SCMI_MSG_IN(SCMI_SENSOR_DESCRIPTION_GET, &desc_in, desc_out);
