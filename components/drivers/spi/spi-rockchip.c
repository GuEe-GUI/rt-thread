...
static void rockchip_spi_set_cs(struct rt_spi_device *spi_dev, rt_bool_t enable)
{
    ...
    {
        en_value = RT_BIT(spi_dev->chip_select);
    }
        ...
        struct rt_spi_configuration *conf)
{
    ...
    if (!conf->max_hz)
    {
        LOG_E("Not max hz in %s", rt_dm_dev_get_name(&device->parent));

        return -RT_EINVAL;
    }

    if (!rk_spi->parent.pins && conf->mode & RT_SPI_CS_HIGH && !rk_spi->cs_high_supported)
    {
        LOG_W("None GPIO CS can't be active-high in %s",
                rt_dm_dev_get_name(&device->parent));
        return -RT_EINVAL;
    }
        ...
        struct rt_spi_message *xfer)
{
    ...
    if ((err = rockchip_spi_config(rk_spi, device, xfer, use_dma, spi_bus->slave)))
    {
        return err;
    }
