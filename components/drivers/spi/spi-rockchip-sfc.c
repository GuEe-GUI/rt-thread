        ...
        rt_ubase_t dma_buf, rt_size_t len)
{
    ...
    rockchip_sfc_writel(rk_sfc, SFC_DMA_TRIGGER_START, SFC_DMA_TRIGGER);
        ...
        struct rt_spi_message *xfer)
{
    ...
    if (xfer->send_buf)
    {
        if ((res = rockchip_sfc_write_fifo(rk_sfc, xfer->send_buf, len)) < 0)
        {
            return res;
        }
    }
    ...
    if (xfer->recv_buf)
    {
        res = rockchip_sfc_read_fifo(rk_sfc, xfer->recv_buf, len);
    }
        ...
        struct rt_spi_device *spi_dev, struct rt_qspi_message *qspi_xfer)
{
    ...
    /* alternate_bytes */
    if (qspi_xfer->alternate_bytes.size)
    {
        if (qspi_xfer->alternate_bytes.qspi_lines == 4)
        {
            cmd |= qspi_xfer->alternate_bytes.size * 2 << SFC_CMD_DUMMY_SHIFT;
        }
        else if (qspi_xfer->alternate_bytes.qspi_lines == 2)
        {
            cmd |= qspi_xfer->alternate_bytes.size * 4 << SFC_CMD_DUMMY_SHIFT;
        }
        else
        {
            cmd |= qspi_xfer->alternate_bytes.size * 8 << SFC_CMD_DUMMY_SHIFT;
        }
    }
    ...
    cmd |= spi_dev->chip_select << SFC_CMD_CS_SHIFT;
...
static void rockchip_sfc_free(struct rockchip_sfc *rk_sfc)
{
    ...
    if (rk_sfc->dma_buffer_cache)
    {
        rt_pages_free(rk_sfc->dma_buffer_cache, rt_page_bits(SFC_MAX_IOSIZE_VER3));
    }

    if (rk_sfc->dma_buffer)
    {
        rt_iounmap(rk_sfc->dma_buffer);
    }
...
static rt_err_t rockchip_sfc_probe(struct rt_platform_device *pdev)
{
    ...
    if (rk_sfc->use_dma)
    {
        rk_sfc->dma_buffer_cache = rt_pages_alloc(rt_page_bits(SFC_MAX_IOSIZE_VER3));
        ...
        rk_sfc->dma_buffer_phy = (rt_ubase_t)rt_kmem_v2p(rk_sfc->dma_buffer_cache);
        rk_sfc->dma_buffer = rt_ioremap_nocache((void *)rk_sfc->dma_buffer_phy, SFC_MAX_IOSIZE_VER3);
